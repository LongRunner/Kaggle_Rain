...
if __name__ == '__main__':
    ...
    tools.save_file('processed_train_2013', data_dict)
    #np.savetxt("processed_train_2013.csv", data_dict['data'], delimiter=",", header=data_dict['labels'])
    '''
    data_dict = tools.load_file('train_2013')
    print(data_dict['data'][:10,0:10])
    data_dict['pca_data'] = tools.pca_norm(np.nan_to_num(data_dict['data'][:,1:19]), k = 10)
